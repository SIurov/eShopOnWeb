...
public class CachedCatalogViewModelService : ICatalogViewModelService
{
    ...
    public async Task<string[]> GetCatalogItemNames(int[] ids)
    {
        return await _cache.GetOrCreateAsync(CacheHelpers.GenerateBrandsCacheKey(), async entry =>
        {
            ...
            return await _catalogViewModelService.GetCatalogItemNames(ids);
        });
    }
    ...
    {
        var cacheKey = CacheHelpers.GenerateCatalogItemCacheKey(pageIndex, Constants.ITEMS_PER_PAGE, brandId, typeId, showEu);

        return await _cache.GetOrCreateAsync(cacheKey, async entry =>
        {
            entry.SlidingExpiration = CacheHelpers.DefaultCacheDuration;
            return await _catalogViewModelService.GetCatalogItems(pageIndex, itemsPage, brandId, typeId, showEu);
        });
    }
